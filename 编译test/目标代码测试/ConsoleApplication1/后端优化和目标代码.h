...
vector< vector<ACT_INF> >Act_inf;//四元式活跃信息表
vector< vector<ACT_SYNBL> >Act_synbl;//活跃信息表
vector<string>objcode;//目标代码串
stack<string>SEM; //保存待返填地址
struct Quaternion qua[maxn];   //四元式数组
vector<Quaternion> Block[15];//单区四元式集合
...
void get_act_inf(vector< vector<Quaternion> >qua)
{
	...
	for (int i = 0; i < (int)qua.size(); i++)
	{
		...
		Act_inf.push_back(act_inf);//将单个基本块的活跃信息表加入整体活跃信息表
	}
